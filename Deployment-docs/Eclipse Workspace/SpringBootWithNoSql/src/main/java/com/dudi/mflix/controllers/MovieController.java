package com.dudi.mflix.controllers;
...
import com.dudi.mflix.models.Movie;
import com.dudi.mflix.services.MoviesService;
...
//    While @ResponseBody puts the return value into the body of the response,
    ...
    public ResponseEntity<Map> moviesByCountry(
            @RequestParam(value = "countries") ArrayList<String> countries) {
    ...
    @PostMapping(value = "/searchByCastAndTitle")
    public ResponseEntity<List<Document>> getMoviesByCastsAndTitles(@RequestBody HashMap<String, List<String>> request)
    {
    	List<String> casts = request.get("casts");
    	List<String> titles = request.get("titles");
    	
    	return ResponseEntity.ok(moviesService.getMoviesByCastAndTitle(casts, titles));
    }
}
